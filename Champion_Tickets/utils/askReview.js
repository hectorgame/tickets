...
module.exports = async(client, channel, guild) => {
  ...
  setTimeout(async() => {
    await user.send({ embeds: [client.embedBuilder(client, user, client.embeds.title, client.language.service.reviews.comment, client.embeds.general_color)] });
  }, 2500);
  ...
  commentCollector.on("collect", async(m) => {
    ...
    if(dataReview.comment == "") {
      ...
      let starCollector = await dm.createMessageComponentCollector({ filter: buttonFilter, componentType: "BUTTON", time: 300000 });
      ...
      starCollector.on("end", async(collected, reason) => {
        ...
        if(reason != "collected" && reason != "canceled") await user.send({ embeds: [client.embedBuilder(client, user, client.embeds.title, client.language.service.reviews.time, client.embeds.success_color)] });
      });
    }
  });
  ...
  commentCollector.on("end", async(collected, reason) => {
    if(reason != "collected" && reason != "canceled") {
      await user.send({ embeds: [client.embedBuilder(client, user, client.embeds.title, client.language.service.reviews.time, client.embeds.success_color)] });
    }
  });
}