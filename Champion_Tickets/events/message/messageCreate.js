...
module.exports = class Message extends Event {
    ...
    if(this.client.utils.permissionsLength(message, message.member, cmd.permissions) > 0 && !this.client.utils.hasRole(this.client, message, this.client.config.roles.bypass.permission)) return message.channel.send({ embeds: [this.client.embedBuilder(this.client, message.author, this.client.embeds.title, this.client.language.general.no_perm, this.client.embeds.error_color)] });
    ...
    if(this.client.cmdConfig[cmd.name]) {
      ...
      if(cmdConfig && cmdConfig.roles.length > 0 && !this.client.utils.hasRole(this.client, message, this.client.config.roles.bypass.permission)) {
        let cmdRoles = cmdConfig.roles.map((x) => this.client.utils.findRole(message.guild, x));
        if(!this.client.utils.hasRole(this.client, message, cmdConfig.roles)) return message.channel.send({ embeds: [this.client.embedBuilder(this.client, message.author, this.client.embeds.title, this.client.language.general.no_role.replace("<role>", cmdRoles.join(", ").trim()), this.client.embeds.error_color)] });
      }
      let findCooldown = cooldownList.find((c) => c.name == cmd.name && c.id == message.author.id);
      if(!this.client.utils.hasRole(this.client, message, this.client.config.roles.bypass.cooldown)) {
        if(findCooldown) {
