...
module.exports = class SetPriority extends Command {
  ...
  async run(message, args) {
    ...
    if(priority == "normal") {
      if(currentPriority) {
        channel.setName(config.channels.channel_name.toLowerCase() + this.client.utils.ticketUsername(ticketOwner));
        db.delete(`priority_${message.guild.id}_${channel.id}`); 
      }
    } else {
      channel.setName(config.channels.priority_name.replace("<priority>", priorityEmojis[priority.toLowerCase()]) + this.client.utils.ticketUsername(ticketOwner));
  ...
  async slashRun(interaction, args) {
    ...
    if(priority == "normal") {
      ...
      channel.setName(config.channels.priority_name.replace(priorityEmojis[priority.toLowerCase()]) + this.client.utils.ticketUsername(ticketOwner));
      db.set(`priority_${message.guild.id}_${channel.id}`, priority); 
    }
